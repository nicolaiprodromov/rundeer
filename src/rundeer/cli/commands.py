...
MAIN_USAGE = """rundeer - batch-first CLI for the Grok Imagine API.
...
from rundeer.core.paths import (
    batch_path as default_batch_path,
...
def _resolve_common(args: Dict[str, Any], config: Dict[str, Any], *, video: bool = False) -> Dict[str, Any]:
    ...
    if raw_output:
        ...
        output_dir = str(default_output_dir(project_root))
...
def _make_client(common: Dict[str, Any]) -> GrokClient:
    ...
    return GrokClient(
        ...
        rate_limit_state_path=rate_limit_state_path(root),
    )
...
def cmd_image(argv: List[str]) -> int:
    ...
    cache = cache_dir(common["project_root"])
...
def cmd_benchmark(argv: List[str]) -> int:
    ...
    if bench_path_arg:
        ...
        bp = benchmark_config_path(project_root)
    if not bp.exists():
    ...
    if template_path_arg:
        ...
        tp = benchmark_dir(project_root, "position", "position.md")
    if not tp.exists():
...
def cmd_batch(argv: List[str]) -> int:
    ...
    if batch_file_arg:
        ...
        batch_path = default_batch_path(Path.cwd())
