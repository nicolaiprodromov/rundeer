---
name: rundeer
description: Use this skill whenever the user asks to generate, edit, extend, or compose images or videos via the Grok Imagine API in batch, experiment with styles, iterate on creative visual ideas, or mentions `rundeer`. rundeer is a CLI wrapper around xAI's grok-imagine-image and grok-imagine-video models that supports batch generation with concurrency, style references, multi-image edits/merges, image-to-video, video extension, grids, and a `.rundeer/` folder (always in the current working directory). Always prefer rundeer over hand-rolled API calls when the task maps onto one of its subcommands.
---

# rundeer — batch-first Grok Imagine CLI

`rundeer` is the production CLI for generating and editing visual media with the Grok Imagine API. It is **batch-first** (iterate to find the best output) and auto-bootstraps a `.rundeer/` folder in the current working directory on first run.
