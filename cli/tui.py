"""Reporters: PlainReporter (fallback) and TuiReporter (curses full-screen)."""
...
CRUSH_LOGO_GRADIENT_FROM: Color = (0xFF, 0x60, 0xFF)  # charmtone.Dolly
CRUSH_LOGO_GRADIENT_TO: Color = (0x6B, 0x50, 0xFF)  # charmtone.Charple
GRADIENT_PAIR_START = 16


class PlainReporter:
    """Simple stdout reporter, used when not a TTY or --no-tui."""
...
class TuiReporter:
    """Curses full-screen reporter with animated deer header, job grid, and log tail."""
    ...
    def _draw(self) -> None:
        with self._lock:
            ...
            # Header: deer frame with a Crush-style horizontal foreground gradient.
    ...
    def _progress_bar(self, state: JobState, width: int) -> str:
        ...
        # indeterminate pulse based on frame index
    ...
    def summary(self, jobs: List[Job], output_dir: Path) -> None:
        # Called after context exit; prints plain text summary
        done = sum(1 for j in jobs if j.state == JobState.DONE)
        failed = sum(1 for j in jobs if j.state == JobState.FAILED)
        print(f"\nSummary: {done} done, {failed} failed / {len(jobs)} total")
        print(f"Output:  {output_dir}")
