"""Persistent API rate limiter for rundeer."""
...
try:  # pragma: no cover - Windows fallback; rundeer primarily runs on Unix.
    import fcntl
...
class RateLimiter:
    ...
    When *state_path* is provided, timestamps are stored in a JSON file and
    protected by an adjacent lock file, so concurrent rundeer processes share
    the same budget. Without *state_path*, the limiter is process-local.
    """
