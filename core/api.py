"""Thin adapter around xai_sdk.Client for mockable API access."""
...
class GrokClient:
    """Wraps xai_sdk.Client with a narrow, testable interface."""
    ...
    def generate_video(
        ...
        self._acquire_request_slot()
        # xAI video API caps prompt at 4096 bytes (UTF-8).
        raw = prompt.encode("utf-8")
        if len(raw) > 4096:
            prompt = raw[:4096].decode("utf-8", errors="ignore")
        params: Dict[str, Any] = {}
        if video_url:
            # Video editing mode: duration/aspect_ratio/resolution are not supported;
            # output inherits these from the input video. Cannot combine with image
