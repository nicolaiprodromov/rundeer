"""Batch orchestration with threaded concurrency and event callbacks."""
...
def _summarize_params(params: Dict[str, Any], max_str: int = 400) -> Dict[str, Any]:
    """Return a copy of *params* with bulky values (data URIs, base64 blobs)
    shortened so they're safe to print under --verbose."""
    out: Dict[str, Any] = {}
...
class BatchRunner:
    ...
    def _run_job(self, job: Job) -> None:
        ...
        # Resolve any "@name" definition references in text-bearing job
        # params just before executing the worker. This is done once per
        # job (not per retry) so the same prompt is used across retries.
...
class _HardError(Exception):
    """Raised by workers to indicate no-retry failures."""
