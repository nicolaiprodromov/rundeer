"""Edit workflow for images and videos."""
...
def build_jobs(
    *,
    ...
    # Video-edit mode: a single input video file routed through xAI's
    # /v1/videos/edits endpoint. Reference images and extra inputs are not
    # supported by the API in this mode.
    ...
    if type_ == "image":
        ...
        if len(input_uris) == 1 and not ref_uris:
            ...
            suffix = ""  # multi-input compose: no style-only framing
        ...
        if video_edit:
            # Edit-video mode: pass source video; duration/aspect_ratio/resolution
            # are ignored by the xAI API (output inherits from input).
            for i in range(1, iterations + 1):
                jobs.append(Job(
                    id=i, kind="video",
                    params={
            ...
            # Image-to-video: first input is starting frame, rest + refs are reference images.
    ...
    return {
        ...
        "type": type_,
        "chain": bool(chain),
        # Context needed to re-encode the previous iteration's output as the
        "chain_ctx": {
            "aspect_ratio": aspect_ratio,
...
def _run_chained(client: GrokClient, plan: Dict[str, Any], reporter) -> None:
    """Run edit jobs sequentially, feeding each output back as the next input.

    With ``batch.chain = true``, iteration N uses iteration N-1's output as
    its input image (or video). The result is a single artifact that has been
    edited ``iterations`` times, rather than ``iterations`` independent edits
    ...
    if compose:
        # Imported lazily so users without compose enabled don't pay the
        # scikit-image import cost.
        from rundeer.core.diff_compose import composite_edit
    else:
        composite_edit = None  # type: ignore[assignment]

    # Cumulative mask of pixels that have been edited at any prior iteration.
    # Future iterations are forbidden from modifying these pixels, which
    # prevents previously-added elements from being re-rendered (and thus
    # degraded) by later edit calls.
    frozen_state: Dict[str, Any] = {"mask": None}

    def chained_worker(job: Job, progress) -> None:
        # The first job keeps its original params (built from --input). For
        # every subsequent job we rewrite the input to point at the previous
        # job's output file.
        idx = jobs.index(job)
        if idx > 0:
            prev = jobs[idx - 1]
            ...
            if plan["type"] == "image":
                ...
                # Image-to-video chaining: feed previous frame/video back as
                # the new starting frame. (Only meaningful if the output is an
                # image; xAI image-to-video produces an mp4, so this branch
                # is effectively unreachable but kept for safety.)
                new_uri = encode_image(prev_path, aspect_ratio,
                                       pad=pad, quality=quality, cache_dir=cache_dir)
                job.params["image"] = new_uri
        base_worker(job, progress)

        # Mask-composite: keep untouched pixels bit-for-bit from the prior
        # image so generational loss can't accumulate, and lock previously
        # edited regions so they can't be re-rendered by later iterations.
